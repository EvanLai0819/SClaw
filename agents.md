# agents.md - Rules for AI Agents Working on SClaw
...
3. **FAST** - The user is waiting in Web UI. Target: first response token in <2 seconds for simple queries. Shared connections, async everything, minimal context window usage. Don't send 50 messages of history when 15 is enough. Don't call LLM to extract memories if the message is "thanks".
...
- The source of truth for architecture is `sclaw-spec-v2.md`. Read it before writing any code.
...
- No emoji in source code or logs. Emoji are allowed ONLY in user-facing Web UI messages and CLI output where explicitly specified in the spec.
...
- User-facing errors should be clear and actionable: "Web search failed: invalid API key. Run `sclaw init` to reconfigure." not "Error occurred."
...
- Test file: `tests/test_<module_name>.py` mirroring `sclaw/<module_name>.py`.
...
This is SClaw's key competitive advantage. OpenClaw bleeds tokens. We don't.
...
- SClaw: sends 4-15 messages, truncated = 1,500-4,000 tokens of history
...
# CLI: sclaw status shows token usage
```

### Token Budget Summary

| Component | OpenClaw (est.) | SClaw | Savings |
|-----------|----------------|----------|---------|
...
SClaw uses two execution patterns, chosen automatically based on task complexity.
...
Target: accepting Web UI messages within 2 seconds of `sclaw serve`.
