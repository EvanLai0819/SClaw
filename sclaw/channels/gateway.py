...
class Gateway:
    ...
    async def start(self) -> None:
        ...
        # Load agent (this triggers skills loading)
        logger.info("Loading agent and skills...")
        _ = self.agent
        logger.info("Agent and skills loaded successfully")

        # Start Web UI if enabled
        if self.config.channels.web_ui.enabled:
            from sclaw.channels.webui import WebUIChannel

            webui = WebUIChannel(self.config.channels.web_ui, self)
            await webui.start(port=self.config.channels.web_ui.port)
            self.channels["webui"] = webui
            logger.info("Web UI channel started")

        # Start cron scheduler
        from sclaw.cron.scheduler import Scheduler
