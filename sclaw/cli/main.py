...
@click.group()
@click.version_option(package_name="sclaw-ai")
def cli() -> None:
    """SClaw - Secure personal AI assistant"""
    pass


@cli.command()
def init() -> None:
    """Initialize SClaw with interactive wizard."""
    asyncio.run(setup_wizard())


async def setup_wizard() -> None:
    """Interactive setup wizard."""
    click.echo("\nWelcome to SClaw setup!\n")
    ...
    # Step 2: Web Search
    click.echo()
    use_search = confirm_interactive("Step 2/3: Enable web search?", default=True)
    if use_search:
        click.echo()
...
SClaw is ready!
  ...
  Start agent:    sclaw serve
  Chat from CLI:  sclaw chat "hello"
  View status:    sclaw status
  Security check: sclaw doctor
"""
    )
...
def chat(message: str | None, verbose: bool) -> None:
    """Chat with SClaw agent."""
    if verbose:
        import os
        os.environ["NANOCLAW_VERBOSE"] = "1"
        from sclaw.core.logger import set_verbose
        set_verbose(True)
    if message:
...
async def one_shot_chat(message: str) -> None:
    ...
    from sclaw.core.agent import get_agent
    from sclaw.core.llm import ConnectionPool
    from sclaw.tools.shell import set_confirm_callback
...
async def interactive_chat() -> None:
    ...
    click.echo("SClaw Interactive Chat")
...
def serve(verbose: bool) -> None:
    """Start SClaw gateway (Telegram + Cron + Dashboard)."""
    if verbose:
        import os
        import logging
        os.environ["SCLAW_VERBOSE"] = "1"
        # Import and set verbose BEFORE any other sclaw imports
        from sclaw.core.logger import set_verbose
        set_verbose(True)
        # Verify logger state
...
async def show_status() -> None:
    ...
    try:
        ...
        click.echo("SClaw not configured. Run 'sclaw init' first.")
        return
    ...
    # Channels
    click.echo(f"\nWeb UI: {'enabled' if config.channels.web_ui.enabled else 'disabled'}")
...
async def add_cron_job(
    ...
    """Add a cron job."""
    from sclaw.cron.scheduler import get_scheduler
...
def config() -> None:
    ...
    config_path = Path.home() / ".sclaw" / "config.json"
    if not config_path.exists():
        click.echo("No config found. Run 'sclaw init' first.")
        return
    ...
    while True:
        ...
        click.echo(f"  Provider: {provider}")
        click.echo(f"  Model: {model}")
        webui_enabled = data.get("channels", {}).get("webUI", {}).get("enabled", False)
        click.echo(f"  Web UI: {'enabled' if webui_enabled else 'disabled'}")
...
def _edit_webui(data: dict, save: Callable[[], None]) -> None:
    """Edit Web UI settings."""
    while True:
        click.echo()
        channels = data.get("channels", {})
        webui = channels.get("webUI", {})

        current_enabled = webui.get("enabled", True)
        current_port = webui.get("port", 18791)
        ...
        options = [
            ("toggle", f"{'Disable' if current_enabled else 'Enable'} Web UI"),
            ("port", "Change port"),
            ("back", "Back"),
        ]
        ...
        if choice == 0:  # Toggle
            webui["enabled"] = not current_enabled
            click.echo(f"  Web UI {'enabled' if webui['enabled'] else 'disabled'}")
        elif choice == 1:  # Port
            click.echo()
            click.echo("  (leave empty to cancel)")
            port = click.prompt("  Port", default="", show_default=False)
            if not port:
                click.echo("  Cancelled.")
                continue
            try:
