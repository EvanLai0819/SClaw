"""Security check command for SClaw installation."""
...
class SecurityDoctor:
    """Comprehensive security check of SClaw installation."""
    ...
    def __init__(self, config_dir: Optional[Path] = None):
        ...
        Args:
            config_dir: Path to SClaw config directory
        """
        self.config_dir = config_dir or Path.home() / ".sclaw"
    ...
    def check_config_permissions(self) -> CheckResult:
        ...
        if not config_path.exists():
            return CheckResult(
                name="Config file",
                passed=False,
                message="config.json not found. Run 'sclaw init' first.",
                severity="critical",
            )
        ...
        return CheckResult(
            name="Config permissions",
            passed=False,
            message="config.json is readable by others. Run: chmod 600 ~/.sclaw/config.json",
            severity="critical",
        )
    ...
    def check_workspace_permissions(self) -> CheckResult:
        ...
        return CheckResult(
            name="Workspace permissions",
            passed=False,
            message="Workspace is accessible by others. Run: chmod 700 ~/.sclaw/workspace",
            severity="warning",
        )
    ...
    def check_webui_config(self) -> CheckResult:
        """Check if Web UI is configured."""
        try:
            from sclaw.core.config import Config

            config = Config.load(self.config_dir / "config.json")

            if config.channels.web_ui.enabled:
                return CheckResult(
                    name="Web UI",
                    passed=True,
                    message=f"Web UI enabled on port {config.channels.web_ui.port}",
                )
            return CheckResult(
                name="Web UI",
                passed=True,
                message="Web UI is disabled",
            )
