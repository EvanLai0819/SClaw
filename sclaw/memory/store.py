...
class MemoryStore:
    ...
    async def save_memory(
        ...
        Updates: refreshes last_accessed for existing facts.
        ...
        def _insert() -> None:
            ...
            # Check for similar existing memories (better deduplication)
            cursor = conn.execute(
                """
            ...
            if existing:
                # Update last_accessed for existing memory instead of duplicating
                conn.execute(
                    """
                    UPDATE memories
...
def get_memory_store() -> MemoryStore:
    ...
    if _memory_store is None:
        from sclaw.core.config import get_data_path

        _memory_store = MemoryStore(get_data_path() / "sclaw.db")
