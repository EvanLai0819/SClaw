...
class ContextBuilder:
    ...
    SKILL_TRIGGERS = {
        "skill_weather": ["weather", "temperature", "rain", "forecast"],
        "skill_github": ["github", "repo", "repository", "pull request", "issue"],
        "skill_news": ["news", "headlines", "latest"],
        "skill_timezones": ["time in", "timezone", "what time"],
        "skill_summarize-url": ["summarize", "summary", "tldr"],
    }
    ...
    def build_system_prompt(self, memories: list[dict]) -> str:
        ...
        return f"""You are SClaw, a friendly and helpful personal AI assistant.
Communicate via Web UI. Be concise, actionable, and engaging.

BEHAVIORS:
1. Be friendly and enthusiastic! Use emojis to make conversations more engaging 😊
2. Bias toward action. Call tools, don't describe what you could do.
3. Minimize iterations. Solve in fewest steps possible.
4. Save important user facts with memory_save.
5. Use spawn_task for tasks over 30 seconds.
6. Match user's language and detail level.
7. Add relevant emojis to make responses more lively and fun! 🎉
8. Follow the must do in the skill markdown file.
...
Time: {current_time}"""
    ...
    def select_tools(
        ...
        # Built-in skills by keyword
        for tool_name, triggers in self.SKILL_TRIGGERS.items():
            if any(t in msg_lower for t in triggers):
                selected_names.add(tool_name)

        # Claude Code skills (knowledge-based) - match by description keywords
        for tool in all_tools:
            tool_name = tool.get("function", {}).get("name", "")
            if tool_name.startswith("skill_") and tool_name not in selected_names:
                description = tool.get("function", {}).get("description", "").lower()
                # Check if any word from description matches user message
                desc_words = self._extract_keywords(description)
                if any(w in msg_lower for w in desc_words):
                    selected_names.add(tool_name)
    ...
    def _extract_keywords(text: str) -> list[str]:
        ...
        chinese_phrases = re.findall(r"[\u4e00-\u9fa5]{2,}", text)
        
        # Split Chinese phrases into individual 2-char segments
        chinese_words = []
        for phrase in chinese_phrases:
            for i in range(len(phrase) - 1):
                segment = phrase[i:i+2]
        ...
        stop_words = {
            ...
            "this", "that", "from", "they", "will", "have", "been",
            "more", "when", "into", "some", "than", "them", "very",
            "just", "over", "such", "your", "about", "would", "which",
            "their", "what", "this", "from", "have", "been", "will",
        }
        
        # Remove common stop words (Chinese)
        chinese_stop_words = {
            "的", "了", "在", "是", "我", "有", "和", "就",
            "不", "人", "都", "一", "一个", "上", "也", "很",
            "到", "说", "要", "去", "你", "会", "着", "没有",
        ...
        keywords = [w for w in words if w not in stop_words and w not in chinese_stop_words]
