...
from sclaw.core.llm import LLMClient, ToolCall
from sclaw.core.logger import setup_logger
from sclaw.memory.store import MemoryStore
from sclaw.security.audit import AuditLog
from sclaw.security.budget import SessionBudget, SessionTracker
from sclaw.security.prompt_guard import PromptGuard
from sclaw.tools.registry import ToolRegistry
...
class Agent:
    ...
    async def run(
        ...
        # Send all tools to LLM (no filtering)
        ...
        for iteration in range(self.max_iterations):
            ...
            try:
                ...
                logger.info(f"  - Tools count: {len(tool_schemas)}")
                logger.info(f"  - Tool names: {[t.get('function', {}).get('name') for t in tool_schemas]}")
                ...
                logger.info(f"  - Content: {llm_response.content[:500] if llm_response.content else 'None'}")
                logger.info(f"  - Tool calls: {[tc.name for tc in llm_response.tool_calls] if llm_response.tool_calls else 'None'}")
            ...
            if llm_response.tool_calls:
                ...
                for tc, result in results:
                    ...
                    logger.info(f"Tool result [{tc.name}]: {result_preview}")
                    messages.append(
                        {
                            "role": "tool",
                            "tool_call_id": tc.id,
    ...
    async def _execute_tool_safely(
        ...
        try:
            ...
            from sclaw.security.sandbox import SecurityError
    ...
    async def _extract_memories(
        ...
        # Skip trivial messages (greetings, short replies, questions)
        skip_patterns = [
            len(user_message) < 20,
            user_message.lower().strip() in [
                "thanks", "thank you", "ok", "okay", "got it", "cool",
                "yes", "no", "sure", "hi", "hello", "hey", "bye",
                "what", "how", "why", "when", "where", "who"
            ],
            user_message.endswith("?"),
            not any(c.isalpha() for c in user_message),
        ]
        
        if any(skip_patterns):
            return

        # Check if message contains personal or factual information
        # More sophisticated triggers than just keywords
        msg_lower = user_message.lower()
        
        # Direct memory requests
        if any(w in msg_lower for w in ["remember", "don't forget", "note that"]):
            pass  # Always extract for explicit memory requests
        
        # Personal information patterns
        elif any(pattern in msg_lower for pattern in [
            "my name", "i am", "i'm", "i work", "my job",
            "i live", "i prefer", "i like", "my project",
            "my company", "my team", "i need", "i want",
            "my email", "my phone", "my address",
            "nickname", "call me", "you can call me", "you can call",
        ]):
            pass  # Extract for personal info
        
        # Technical/project context
        elif any(pattern in msg_lower for pattern in [
            "using", "working on", "building", "developing",
            "stack", "framework", "language", "database",
            "deploy", "server", "api", "endpoint",
        ]):
            pass  # Extract for technical context
        
        # Preferences and settings
        elif any(pattern in msg_lower for pattern in [
            "prefer", "like", "hate", "love", "usually",
            "always", "never", "default", "configuration",
        ]):
            pass  # Extract for preferences
        
        else:
            # Skip for generic queries and follow-ups
            return
        ...
        try:
            extract_prompt = [
                {
                    "role": "system",
                    "content": (
                        "Extract ONLY factual, useful information about the user from this "
                        "conversation. Return a JSON array of strings (max 3 facts).\n\n"
                        "Guidelines:\n"
                        "- Extract: name, nickname, job, preferences, tech stack, project names, "
                        "contact info, timezone, work hours, team size, hobbies, interests\n"
                        "- Skip: greetings, opinions without context, "
                        "temporary states ('i'm busy now'), trivial exchanges\n"
                        "- Be concise: 10-20 words per fact\n"
            ...
            # Save each fact with deduplication check
            for fact in facts[:3]:
...
def get_agent() -> Agent:
    ...
    global _agent
    if _agent is None:
        from sclaw.core.config import get_config
        from sclaw.core.llm import get_llm_client
        from sclaw.core.logger import setup_logger
        from sclaw.memory.store import get_memory_store
        from sclaw.security.audit import get_audit_log
        from sclaw.security.budget import get_session_budget
        from sclaw.security.prompt_guard import get_prompt_guard
        from sclaw.tools.registry import get_tool_registry

        # Setup global logger with file output
        config = get_config()
        log_config = None
        ...
        # Setup global sclaw logger
        setup_logger("sclaw", log_config=log_config)

        # Import tools to register them
        import sclaw.tools.files  # noqa: F401
        import sclaw.tools.memory_tools  # noqa: F401
        import sclaw.tools.shell  # noqa: F401
        import sclaw.tools.spawn  # noqa: F401
        import sclaw.tools.web  # noqa: F401
