...
class ChannelsConfig(BaseModel):
    ...
    web_ui: WebUIConfig = Field(default_factory=WebUIConfig, alias="webUI")
...
class Config(BaseModel):
    ...
    def load(cls, config_path: Optional[Path] = None) -> Config:
        ...
        if config_path is None:
            # First try local config in project directory
            local_config = Path("test_config.json")
        ...
        if not config_path.exists():
            raise FileNotFoundError(
                f"Config not found at {config_path}. Run 'sclaw init' first."
            )
    ...
    def get_default_model(self) -> str:
        ...
        if self.agents.defaults.model:
            ...
            logger.info(f"Using model from agents.defaults: {self.agents.defaults.model}")
            return self.agents.defaults.model
        _, _, model, _ = self.get_active_provider()
        logger = get_logger(__name__)
        logger.info(f"Using model from provider1: {model}")
        print(f"Using model from provider1: {model}")
        return model
...
def get_workspace_path() -> Path:
    ...
    return Path.home() / ".sclaw" / "workspace"


def get_data_path() -> Path:
    """Get the data directory path."""
    data_dir = Path.home() / ".sclaw" / "data"
