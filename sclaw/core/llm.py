...
class LLMClient:
    ...
    def _build_headers(self) -> dict[str, str]:
        ...
        if self.provider == "openrouter":
            headers["HTTP-Referer"] = "https://github.com/sclaw/sclaw"
            headers["X-Title"] = "SClaw"
        return headers
...
def get_llm_client() -> LLMClient:
    ...
    if _llm_client is None:
        ...
        logger.info(f"LLM Client: api_key={'*' * 20 if api_key else 'None'}")
        logger.info(f"LLM Client: base_url={base_url}")
